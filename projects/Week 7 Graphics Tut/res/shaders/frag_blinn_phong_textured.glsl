...
void main() {
	...
	//vec4 textureColor1 = texture(s_Diffuse1, inUV);
	vec4 textureColor = mix(texture(s_Diffuse, inUV), texture(s_Diffuse1, inUV), u_TextureMix);

	//vec4 textureColor2 = texture(s_Diffuse1, inUV) * texture(s_Diffuse, inUV);
	//float resultCompleted = (1.0f - TextureMix) * diffuse + TextureMix * diffuse1;
